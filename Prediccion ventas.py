    ...
    def load_csv(self, path: str) -> tuple[bool, str, pd.DataFrame | None]:
        try:
            ...
            if not cols_req.issubset(set(df.columns.str.lower())):
                return False, f"El archivo debe tener las columnas: {cols_req}", None
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.nb.add(self.tab_bd,         text="  🗄️  Base de Datos  ")
    ...
    def _build_prediccion(self):
        ...
        self.cmb_degree = ttk.Combobox(ctrl,
                                       values=["1 - Lineal", "2 - Cuadrática", "3 - Cúbica"],
        ...
        self._placeholder(self.frm_pred_chart,
                          "Configura los parámetros y presiona 'Generar Predicción'")
    ...
    def _build_bd(self):
        ...
        sb_bd = ttk.Scrollbar(left, orient="vertical", command=self.tree_bd.yview)
        ...
        # Panel derecho: estadísticas del archivo seleccionado
        ...
        tk.Label(right, text="📊  Estadísticas del archivo",
    ...
    def _bd_stats_placeholder(self):
        ...
        tk.Label(self.frm_bd_stats,
                 text="Selecciona un archivo para ver sus estadísticas",
                 bg=COLORS["panel"], fg=COLORS["text_dim"],
    ...
    def _bd_refresh(self):
        ...
        for arch in self.db.listar_archivos():
            self.tree_bd.insert("", "end", iid=str(arch.id), values=(
                ...
                arch.fecha_carga.strftime("%d/%m/%Y %H:%M") if arch.fecha_carga else "—",
            ))
    ...
    def _on_bd_select(self, event):
        ...
        stat_items = [
            ...
            ("Productos únicos",   str(stats["productos"])),
            ("Fecha inicio",       str(stats["fecha_min"])[:10] if stats["fecha_min"] else "—"),
            ("Fecha fin",          str(stats["fecha_max"])[:10] if stats["fecha_max"] else "—"),
        ]
    ...
    def _bd_cargar(self):
        ...
        if df.empty:
            messagebox.showerror("Error", "No se encontraron datos para ese archivo.")
            return
        ok, msg = self.model.load_dataframe(df)
        if ok:
            arch   = next((a for a in self.db.listar_archivos() if a.id == archivo_id), None)
            nombre = arch.nombre if arch else f"ID {archivo_id}"
            self.lbl_file.config(text=f"🗄️ BD: {nombre}")
    ...
    def _bd_eliminar(self):
        sel = self.tree_bd.selection()
        if not sel:
            messagebox.showinfo("Aviso", "Selecciona un archivo para eliminar.")
            return
        archivo_id = int(sel[0])
        confirm = messagebox.askyesno(
            "Confirmar eliminación",
            f"Se eliminará el archivo ID {archivo_id} y todos sus registros.\n"
    ...
    def _reload(self):
        ...
        if not path or path == "Sin archivo cargado" or path.startswith("🗄️"):
            messagebox.showinfo("Info", "No hay archivo CSV en disco para recargar.")
            return
        self._apply_file(path)
